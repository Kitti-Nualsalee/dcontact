...
6. **ผลลัพธ์** — รับ = `ACTIVE`; ring timeout = ปล่อย reservation, ข้าม agent นั้นชั่วคราว,
   วนกลับข้อ 4 (ลูกค้าไม่รู้สึก — ยังอยู่ในคิว); ลูกค้าวางก่อน = `ABANDONED`
...
| กติกา | ทำไม |
|---|---|
| เก็บเป็น **user preference ผูกกับคน** ไม่ใช่ tenant config | [ADR-005](adr/005-multitenant-metadata-architecture.md) คุมการปรับแต่งของ *องค์กร* — ความกว้างคอลัมน์เป็นรสนิยมของ *คนใช้* mockup เก็บใน `localStorage`, ของจริงต้องเก็บฝั่ง server (เสนอ `users.prefs Json`) เพื่อให้ย้ายเครื่องหรือเปลี่ยนแท็บแล้วได้หน้าจอเดิม |
...
**ข้อความขาออกไม่มีตาราง outbox แยก** — แถวใน `messages` ที่สถานะ `QUEUED` *คือ* outbox
